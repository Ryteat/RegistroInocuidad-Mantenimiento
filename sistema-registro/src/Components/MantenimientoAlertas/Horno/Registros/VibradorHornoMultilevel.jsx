...
const POSICION_ID_BASE = "H-HM-V"; // se completará como H-HM-V-01
...
export default function VibradorHornoMultilevel() {
        ...
        try {
            ...
            const respuestasPayload = Object.fromEntries(
                Array.from({ length: 14 }, (_, i) => {
                    const idx = i + 1;
                    if (idx <= Q_MENSUAL.length) {
                        return [
                            `respuesta_q${idx}`,
                            form.ejecutado === "SI"
                                ? form.items[`q${idx}`]?.respuesta || null
                                : null,
                        ];
                    }
            ...
            if (editingId) {
                ...
                if (form.ejecutado === "SI" && todasSi) {
                    // ✅ COMPLETADO: todas las respuestas en "SI"
                    updatePayload = {
                        ...updatePayload,
                    ...
                    // MENSUAL → +1 mes, sigue "pendiente" hasta que esté todo SI
        ...
        <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
            <span className="p-input-icon-left">
                ...
                <InputText
                    ...
                    placeholder="Buscar (H-HM-V-01, técnico, notas)"
                />
            </span>
    ...
    const countSiNo = (row, val) =>
        Array.from({ length: Q_MENSUAL.length }, (_, i) => i + 1).reduce(
            (acc, i) =>
    ...
    const exportXlsx = () => {
        ...
        XLSX.writeFile(
            wb,
            `Horno_VibradorMultilevel_${new Date().toISOString().slice(0, 10)}.xlsx`
        );
    };
    ...
    return (
        <div className="controlrendcosechayfrass-container">
            ...
            <Dialog
                ...
                header={
                    editingId
                        ? "Editar registro — Vibrador (Horno Multilevel)"
                        : "Nuevo registro — Vibrador (Horno Multilevel)"
                }
                modal
                onHide={hideDialog}
                footer={
