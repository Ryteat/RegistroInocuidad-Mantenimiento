// src/Components/MantenimientoAlertas/Horno/Registros/SelladoraBandaContinuaGeneral.jsx
...
const POSICION_ID = "H-SBC-G";                  // ID
const EQUIPO = "SELLADORA DE BANDA CONTINUA";   // Equipo
const REGISTRO = "GENERAL";                     // Registro
const TABLE = "mto_horno_selladora_banda_continua_general";
...
/* ===== Preguntas EXACTAS (transcritas de tus OMs) ===== */
/* Semestral (2) */
const Q_SEMESTRAL = [
    { key: "q1", label: "Verifique el estado de desgaste de la escobilla de carbón del motor. Reporte" },
    { key: "q2", label: "Adicionar grasa lubricante a los engranajes que están expuestos sin tapa" },
];
/* Mensual (3) */
const Q_MENSUAL = [
    { key: "q1", label: "Compruebe si la correa de sellado está desgastada o no" },
    { key: "q2", label: "Compruebe si la correa guía de sellado está desgastada o no" },
    { key: "q3", label: "Limpiar los carbones del motor" },
];
/* Anual (5) */
const Q_ANUAL = [
    { key: "q1", label: "Verifique exhaustivamente la máquina" },
    { key: "q2", label: "Compruebe si la rueda de goma está deteriorada, reemplace si es necesario" },
    { key: "q3", label: "Verifique que el controlador de temperatura alcance el valor seteado en 10 minutos, de lo contrario cambie el controlador." },
    { key: "q4", label: "Compruebe si las piezas electrónicas, paro de emergencia, potenciómetro, ventilador, botones funcionan correctamente." },
    { key: "q5", label: "Revise y engrase las partes de transmisión, engranajes, ejes y juntas. Cambie si es necesario. Limpie la caja de engranajes y cambie el aceite" },
];
...
export default function SelladoraBandaContinuaGeneral() {
    ...
    const onPeriodoChange = (value) => {
        const base = value === "SEMESTRAL" ? Q_SEMESTRAL : value === "MENSUAL" ? Q_MENSUAL : Q_ANUAL;
    ...
    const validate = () => {
        ...
        if (form.ejecutado === "SI") {
            const list = form.periodicidad === "SEMESTRAL" ? Q_SEMESTRAL : form.periodicidad === "MENSUAL" ? Q_MENSUAL : Q_ANUAL;
        ...
        try {
            ...
            const proximo =
                form.ejecutado === "NO" ? addDays(baseDate, 7)
                    : form.periodicidad === "SEMESTRAL" ? addMonths(baseDate, 6)
                        : form.periodicidad === "MENSUAL" ? addMonths(baseDate, 1)
    ...
    /* -------- revisado -------- */
    const { canReview: canR, username: user } = useCanReview(); // compat
    const revisadoTemplate = (row) => {
        const allow = (canR ?? canReview);
        const uname = (user ?? username);
        if (!allow) return <span>{row.revisado ? "Sí" : "No"}</span>;
    ...
    const header = (
        <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
            <span className="p-input-icon-left">
                ...
                <InputText type="search" value={globalFilter} onInput={(e) => setGlobalFilter(e.target.value)} placeholder="Buscar (H-SBC-G, técnico, notas)" />
            </span>
    ...
    const countSiNo = (row, val) => Array.from({ length: 14 }, (_, i) => i + 1)
    ...
    const exportXlsx = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Selladora-BC General");
        XLSX.writeFile(wb, `Horno_SelladoraBandaContinua_General_${new Date().toISOString().slice(0, 10)}.xlsx`);
    };

    const activeQuestions =
        form.periodicidad === "SEMESTRAL" ? Q_SEMESTRAL :
            form.periodicidad === "MENSUAL" ? Q_MENSUAL :
    ...
    return (
        <div className="controlrendcosechayfrass-container">
            ...
            <h1 className="flex align-items-center gap-2">
                ...
                Registro — Selladora de Banda Continua (General)
            </h1>
            ...
            <Dialog visible={dialogOpen} style={{ width: "72vw", maxWidth: 1100 }}
                header="Nuevo registro — Selladora de Banda Continua (General)" modal onHide={hideDialog}
                ...
                <div className="p-fluid grid">
                    <div className="field col-12 md:col-4">
                        ...
                        <small className="block mt-2">
                            <b>Mensual</b>: +1 mes; <b>Semestral</b>: +6 meses; <b>Anual</b>: +12 meses.
                        </small>
                    </div>
                        ...
                        <div className="field col-12">
                            <div style={{ border: "1px solid #d1d5db", borderRadius: 8, padding: 12 }}>
                                <div style={{ fontWeight: 700, marginBottom: 8 }}>
                                    {form.periodicidad === "SEMESTRAL" ? "Checklist — SEMESTRAL"
                                        : form.periodicidad === "MENSUAL" ? "Checklist — MENSUAL" : "Checklist — ANUAL"}
                                </div>
                                <div className="grid">
